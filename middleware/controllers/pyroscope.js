...
const { convertPprofToMarkdown, convertPprofToMarkdownCustom } = require("../utils/pprofConverter")
...
/**
 * Converts profiling data from Pyroscope to markdown format without analysis.
 *
 * @async
 ...
 * @param {string} [req.body.from="now-5m"] - The start time for the profiling data range (default: 5 minutes ago).
 * @param {string} [req.body.until="now"] - The end time for the profiling data range (default: now).
 * @param {string} [req.body.format="pprof"] - The format of the profiling data (default: pprof).
 * @param {import('express').Response} res - The Express response object.
 * @returns {Promise<void>} Sends the markdown data as a response or an error response.
 */
async function getProfileMarkdown(req, res) {
    const { query, from = "now-5m", until = "now", format = "pprof", test = false } = req.body;
    const baseUrl = buildUrl(getEnv("PYROSCOPE_BASE_URL"), {
      query: `${query}.cpu`,
      from: from,
    ...
    try {
      ...
      try {
        const markdown = await convertPprofToMarkdownCustom(profilePath);
        return res.type("text/markdown").send(markdown);
      } catch (conversionError) {
        logger.error('Error converting profile to markdown:', conversionError);
        return res.send({
